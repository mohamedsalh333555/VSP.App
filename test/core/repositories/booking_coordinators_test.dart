...
void main() {
  ...
  group('MockBookingRepository Tests', () {
    ...
    test('initial state has no bookings', () async {
      final bookings = await repository.getUserBookingsDirectly('user_1');
      expect(bookings, isEmpty);
    });

    test('addMockBookings populates repository', () async {
      repository.addMockBookings();
      final userBookings = await repository.getUserBookingsDirectly('demo_user');
      expect(userBookings.length, 2);
    });

    test('getBookingById finds correct booking', () async {
      repository.addMockBookings();
      final booking = await repository.getBookingById('mock_1');
      expect(booking, isNotNull);
      expect(booking?.stadiumName, 'Santiago Bernabéu');
    });

    test('updateBookingStatus updates status properly', () async {
      repository.addMockBookings();
      final updated = await repository.updateBookingStatus('mock_1', BookingStatus.cancelled);
    ...
    test('updatePaymentStatus toggles paid flag', () async {
      repository.addMockBookings();
      final updated = await repository.updatePaymentStatus('mock_1', true);
      expect(updated, isTrue);
    ...
    test('createBooking creates and stores new booking', () async {
      ...
      final created = await repository.createBooking(draft, 'player_777');
      expect(created.id, startsWith('mock_'));
      expect(created.stadiumName, 'Test Stadium');
      expect(created.createdByUserId, 'player_777');

      final found = await repository.getBookingById(created.id);
