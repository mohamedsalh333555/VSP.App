...
void main() {
  group('FeatureFlagService Tests', () {
    ...
    test('Defaults provide safe offline values', () {
      expect(service.isOnlinePaymentEnabled, isFalse);
      expect(service.isMaintenanceMode, isFalse);
      expect(service.isEnabled('is_championships_enabled'), isFalse);
    });
    ...
    test('Handles fallback for non-existent flags gracefully', () {
      ...
      expect(service.getInt('non_existent_int', defaultValue: 42), equals(0));
      expect(service.getString('non_existent_str', defaultValue: 'default_val'), equals(''));
    });
  });
}
