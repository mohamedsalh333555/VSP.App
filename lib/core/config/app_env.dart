...
class AppEnv {
  static String get supabaseUrl {
    ...
    if (kReleaseMode) {
      VSPLogger.w('⚠️ Production build running with fallback SUPABASE_URL.');
    }
  ...
  static String get supabaseAnonKey {
    ...
    if (kReleaseMode) {
      VSPLogger.w('⚠️ Production build running with fallback SUPABASE_ANON_KEY.');
    }
  ...
  static String get paymobPublicKey {
    ...
    if (key.isNotEmpty) return key;
    if (kReleaseMode) {
      VSPLogger.w('⚠️ Production build running with fallback PAYMOB_PUBLIC_KEY.');
    }
