...
class BookingMapper {
  ...
  static Booking fromFirestore(Map<String, dynamic> data, String id) {
    ...
    final matchResultStatusVal =
        data['matchResultStatus'] ?? data['match_result_status'];
    ...
    final int ppt = _parseInt(data['players_per_team'] ?? data['playersPerTeam'], 5);
    final int tfc = _parseInt(
        data['total_field_capacity'] ??
        data['totalFieldCapacity'] ??
        data['maxPlayers'] ??
        data['max_players'],
        ppt * 2);
    ...
    return Booking(
      ...
      stadiumImageUrl:
          data['stadiumImageUrl'] ?? data['stadium_image_url'] ?? '',
      ...
      playerTeamLogoUrl:
          data['playerTeamLogoUrl'] ?? data['player_team_logo_url'],
      hostName: data['hostName'] ?? data['host_name'],
      hostAvatarUrl: data['hostAvatarUrl'] ?? data['host_avatar_url'],
      opponentTeamId: (bookingTypeVal == 'challenge')
          ? (data['opponentTeamId'] ?? data['opponent_team_id'])
          : null,
      opponentTeamName: (bookingTypeVal == 'challenge')
          ? (data['opponentTeamName'] ?? data['opponent_team_name'])
          : null,
      opponentTeamLogoUrl:
          data['opponentTeamLogoUrl'] ?? data['opponent_team_logo_url'],
      isPrivate: data['isPrivate'] ?? data['is_private'] ?? false,
      rentBall: data['rentBall'] ?? data['rent_ball'] ?? false,
      totalPrice: _parseDouble(data['totalPrice'] ?? data['total_price'], 0.0),
      currency: data['currency'] ?? 'EGP',
      paymentMethod: data['paymentMethod'] ?? data['payment_method'] ?? 'card',
      paymentTransactionId:
          data['paymentTransactionId'] ?? data['payment_transaction_id'],
      status: () {
        if (statusVal == 'upcoming') return BookingStatus.confirmed;
        final matched = BookingStatus.values.where((e) => e.name == statusVal);
        if (matched.isNotEmpty) {
          return matched.first;
        }
        VSPLogger.e(
          'Critical: Unknown booking status encountered: "$statusVal" for booking ID: $id. '
          'Falling back to pending with observability.',
        );
      ...
      homeScore: (data['homeScore'] ?? data['home_score']) != null
          ? _parseInt(data['homeScore'] ?? data['home_score'])
          : null,
      awayScore: (data['awayScore'] ?? data['away_score']) != null
          ? _parseInt(data['awayScore'] ?? data['away_score'])
          : null,
      resultSubmittedByTeamId:
          data['resultSubmittedByTeamId'] ??
          data['result_submitted_by_team_id'],
      ...
      paymentStatus:
          data['paymentStatus'] ??
          data['payment_status'] ??
          ((data['isPaid'] ?? data['is_paid']) == true ? 'paid' : 'pending'),
      playerPhone: data['playerPhone'] ?? data['player_phone'],
      notes: data['notes'],
      cancellationReason:
          (data['cancellation_reason'] ?? data['cancellationReason'])
              ?.toString(),
      depositPaid: _parseDouble(
          data['deposit_paid'] ?? data['depositPaid'], 0.0),
      ...
      lastMessageTime:
          (data['last_message_time'] ?? data['lastMessageTime']) != null
          ? DateTime.parse(
              (data['last_message_time'] ?? data['lastMessageTime']).toString(),
            )
          : null,
      rescheduleStatus:
          data['reschedule_status'] ?? data['rescheduleStatus'] ?? 'none',
      proposedStartTime:
          (data['proposed_start_time'] ?? data['proposedStartTime']) != null
          ? DateTime.parse(
              (data['proposed_start_time'] ?? data['proposedStartTime'])
                  .toString(),
            )
          : null,
      proposedEndTime:
          (data['proposed_end_time'] ?? data['proposedEndTime']) != null
          ? DateTime.parse(
              (data['proposed_end_time'] ?? data['proposedEndTime']).toString(),
            )
          : null,
      emergencyCancelStatus:
          data['emergency_cancel_status'] ??
          data['emergencyCancelStatus'] ??
          'none',
      emergencyReason: data['emergency_reason'] ?? data['emergencyReason'],
      emergencyDowntimeHours: (data['emergency_downtime_hours'] ??
                  data['emergencyDowntimeHours']) !=
              null
          ? _parseInt(
              data['emergency_downtime_hours'] ??
                  data['emergencyDowntimeHours'])
          : null,
      refundAmount: (data['refund_amount'] ?? data['refundAmount']) != null
          ? _parseDouble(data['refund_amount'] ?? data['refundAmount'])
          : null,
      refundTransactionId:
      ...
      refundedAt: (data['refunded_at'] ?? data['refundedAt']) != null
          ? ((data['refunded_at'] ?? data['refundedAt']) is DateTime
                ? ((data['refunded_at'] ?? data['refundedAt']) as DateTime)
                      .toLocal()
                : DateTime.tryParse(
                    (data['refunded_at'] ?? data['refundedAt']).toString(),
                  )?.toLocal())
          : null,
      refundPaymentMethod:
          (data['refund_payment_method'] ?? data['refundPaymentMethod'])
              ?.toString(),
      refundChannel: (data['refund_channel'] ?? data['refundChannel'])
          ?.toString(),
  ...
  static Map<String, dynamic> toFirestore(Booking booking) {
    return {
      ...
      'operational_date': booking.operationalDate
          ?.toIso8601String()
          .split('T')
          .first,
      ...
      if (booking.refundedAt != null)
        'refunded_at': booking.refundedAt?.toUtc().toIso8601String(),
