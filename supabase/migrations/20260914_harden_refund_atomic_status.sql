-- ==============================================================================
-- MIGRATION: 20260914_harden_refund_atomic_status.sql
-- DESCRIPTION: Prevent cancel_booking_with_refund_atomic from marking online payments
--              as 'refunded' without actual gateway confirmation. Set to 'refund_pending'
--              and log pending transaction until Paymob API or webhook confirms.
-- ==============================================================================
...
BEGIN
    ...
    IF v_booking.payment_status = 'paid' OR v_booking.payment_status = 'confirmed' OR v_booking.is_paid = true OR v_booking.is_deposit_paid = true THEN
        v_refund_amount := COALESCE(v_booking.deposit_paid, v_booking.deposit_amount, v_booking.total_price, 0.00);
    END IF;

    -- Update booking status to cancelled.
    -- 🛡️ CRITICAL FIX: If an online refund is due, do NOT mark as 'refunded' in SQL without calling Paymob API.
    -- Set to 'refund_pending' so Edge Function or Admin processes the actual gateway refund.
    ...
    -- Record refund transaction as 'pending' if online payment is due for gateway processing
