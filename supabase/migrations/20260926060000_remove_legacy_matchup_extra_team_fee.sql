-- Remove legacy sixth-team fee logic from Matchup.
-- VSP Matchup supports at most five teams; there is no separate 25 EGP
-- "extra team" fee in the current product rules.
...
BEGIN
  ...
  SELECT id, created_by_user_id, user_id, status, matchup_closed_at, end_time
    INTO v_booking
  ...
  IF v_booking.matchup_closed_at IS NOT NULL
     OR v_booking.status = 'cancelled'
     OR v_booking.end_time <= timezone('utc', now()) THEN
    RAISE EXCEPTION 'Cannot add teams to an ended or closed matchup';
  END IF;
  ...
  SELECT id, name, logo_url, captain_id, invite_code_expires_at
    INTO v_target_team
    FROM public.teams
  ...
  IF v_target_team.invite_code_expires_at IS NOT NULL
     AND v_target_team.invite_code_expires_at < timezone('utc', now()) THEN
  ...
  IF v_members < 5 THEN
    RAISE EXCEPTION 'Team is incomplete (less than 5 members)';
  END IF;
  ...
  IF EXISTS (
    SELECT 1
     ...
     WHERE booking_id = p_booking_id
       AND team_id = v_target_team.id
  ) THEN
    RAISE EXCEPTION 'Team is already added to this matchup';
  END IF;
  ...
  IF v_current_count >= 5 THEN
    RAISE EXCEPTION 'MATCHUP_TEAM_LIMIT_REACHED: A matchup can contain at most 5 teams';
  END IF;

  INSERT INTO public.matchup_teams (
    booking_id, team_id, added_by_user_id, joined_at
  )
  VALUES (
    p_booking_id, v_target_team.id, v_caller_id, timezone('utc', now())
  );
